...
class transfer_bar:
    ...
    def progress(self, counter):
        ...
        return self.bar(counter) + ' ' + self.rate(counter) +  \
                   ', ETA: ' + self.time_left(counter) + ", " + \
                   size_str(counter) + " / " + size_str(self.expected_size)
...
def download_url(url, file_name,
                 ca_file = CA_FILE, chunk_size = CHUNK_SIZE):
...
class disk:
    ...
    def __init__(self, disk_info, disk_service, oh,
...
#        client.upload(filename, transfer.transfer_url, self.ca_file)
        upload_url(transfer.transfer_url, filename,
                   ca_file = self.ca_file, chunk_size = self.chunk_size)
...
class snapshot_disk(disk):
    ...
    def __str__(self):
        return 'Snapshot disk %s with id: %s' \
                % (self.name(), self.image_id())
    ...
    def download(self, download_dir = DOWNLOAD_DIRECTORY):
        ...
        download_url(transfer.transfer_url, file_name,
                     ca_file = self.ca_file, chunk_size = self.chunk_size)
...
class snapshot:
    ...
    def all_disks(self):
        ...
        for disk_info in disks:
            ...
            all_disks.append(
                snapshot_disk(disk_info, disk_service, self.oh)
                )
...
class VM:
    ...
    def all_snapshots(self, omit_active = True):
        ...
        for snapshot_info in snapshots:
            ...
            all_snapshots.append(
                snapshot(snapshot_info, snapshot_service, self.oh)
                )
    ...
    def download_snapshot_disks(self, download_dir = DOWNLOAD_DIRECTORY):
        l.info('Downloading vm disks for all snapshots for vm %s...' %self.name() )
    ...
    def add_disk(self,disk_name = NEW_DISK_NAME,
    ...
    def settings(self):
        ...
        for snapshot in self.all_snapshots(omit_active = True):
            ...
            l.debug('Discovered snapshot with id %s and description %s' %( snapshot.id(), snapshot.description() ) )
    ...
    def add_disk_snapshots2(self, settings, storage_domain = STORAGE_DOMAIN):
        ...
        for snapshot in settings['snapshot_sequence']:
            ...
            for base_disk in base_disks:
                ...
                l.debug('Included base image with id: %s as a new disk with id %s (image id:%s) in snapshot attachments' \
                    %( base_disk['id'], new_disk.id(), new_disk.image_id() ) )
            ...
            self.add_snapshot(description = description,
                              disk_attachments = disk_attachments)
...
def qemu_rebase(filename, new_base, format):
    s = subprocess.check_output(['qemu-img', 'rebase', '-u', filename, '-b', new_base, '-F', format])
    return s
...
def qemu_chains(directory, filenames = '*'):
    ...
    for ancestor in ancestors:
        ...
        child_list = [ k for k, x in depths.items() if (x['ancestor'] == ancestor) and ( x['depth'] == max(l) ) ]
...
class ovirt_handler:
    ...
    def __init__(self, url = URL,
        ...
        self.connection = sdk.Connection(url = url,
                                         username = username,
                                         ca_file = ca_file,
                                         password = password)
    ...
    def add_vm_from_settings(self, settings,
