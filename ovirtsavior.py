...
def parse_arguments():
    ...
    parser.add_argument("mode", metavar = "mode", type = str, help = "backup or restore: specifies backup or restore mode.")
    parser.add_argument("-s","--setup_file", metavar = "setupfile", help = "setup file containing all the paremeters.", required = True)
...
class savior_job:
    ...
    def execute(self):
        ...
        if self.mode == 'backuptemp':
            ...
            self.oh.add_vm_from_settings(self.vm_settings,
    ...
    def check_missing(self,required):
        ...
        if len(missing) != 0:
            msg = 'Did not find any values for parameter(s) %s in the config file' % ', '.join(missing)
            raise ValueError(msg)
    ...
    def connect_to_api(self):
        ...
        try:
            self.oh = ovirt_handler(url = self.params['ovirt_url'],
    ...
    def check_backup_directory(self):
        ...
        if os.path.isdir(self.working_directory):
            l.warning('Directory %s already exists. Contents may be overwritten.' %self.working_directory)
        else:
    ...
    def download_disks(self):
        ...
        self.vm.download_snapshot_disks(download_dir = self.working_directory)
    ...
    def copy_to_local(self):
        ...
        l.info('Copying discs from working directory %s to temp directory %s' %(working_directory, local_directory) )
    ...
    def check_for_restored_vm(self):
        if self.oh.get_vm_by_name(self.new_vm_name):
            raise ValueError('A VM with name %s already exists in the cluster. Consider removing it. Terminating.' %self.new_vm_name)
